...
<head>
    ...
    <style>
        ...
        .btn-logout {
            ...
            background-color: #be93d4 !important;
            border: none;
            cursor: pointer;
        ...
        /* Style untuk Sidebar Menu Utama */
        .sidebar-link {
            display: flex;
            align-items: center;
        ...
        /* Style Khusus saat Menu Aktif (Mencegah eror linter akibat kode bercampur) */
        .sidebar-link.active {
                ...
                <div class="flex items-center space-x-6 text-sm font-semibold">
                    ...
                    <div class="flex items-center space-x-3 border-l border-gray-200 pl-6">
                        <form action="{{ route('logout') }}" method="POST" style="display: inline;">
                            @csrf
                            <button type="submit" class="btn-logout px-5 py-2 rounded-lg text-xs font-black uppercase shadow-sm">
                                Keluar
                            </button>
                        </form>
                    </div>
...
<div class="flex flex-col items-center mb-6">
    <div class="w-20 h-20 rounded-full border-4 border-[#7a4988] overflow-hidden shadow-md">
        @if(session('admin_foto'))
            <img src="{{ asset('images/' . session('admin_foto')) }}" class="w-full h-full object-cover">
        @else
            <img src="https://ui-avatars.com/api/?name={{ urlencode(session('admin_name', 'Admin')) }}&color=ffffff&background=7a4988" class="w-full h-full object-cover">
        @endif
    </div>
</div>
                ...
                {{-- NAVIGASI SIDEBAR (Sudah Bersih dari inline code pemicu eror) --}}
                <nav class="w-full space-y-2 px-4">
                    {{-- KELOLA ACARA --}}
                    <a href="{{ route('admin.dashboard') }}" 
                       class="sidebar-link {{ request()->routeIs('admin.dashboard') || request()->routeIs('admin.acara.*') ? 'active' : '' }}">
