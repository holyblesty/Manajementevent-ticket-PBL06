    ...
    <div class="bg-white pb-8" id="eventList">
        ...
        <a href="{{ route('admin.peserta.detail', $event->id) }}"
           ...
           class="event-link flex items-center justify-between px-10 py-8 border-b border-gray-100 hover:bg-gray-50 transition no-underline group">
            ...
            <div class="shrink-0">
                <span class="inline-flex items-center justify-center px-8 py-4 rounded-xl border-2 text-2xl font-black shadow-sm {{ $event->is_full ? 'bg-red-50 text-red-700 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200' }}">
                    {{ $event->total_pendaftar }} <span class="mx-2 text-gray-400 font-bold">/</span> {{ $event->kuota }} <span class="ml-2 text-sm uppercase tracking-widest text-gray-500">{{ $event->label }}</span>
                </span>
            </div>
        </a>
        @empty
