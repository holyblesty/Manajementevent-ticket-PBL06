...
<div class="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
    ...
    {{-- LIST EVENT --}}
    <div class="bg-white pb-5" id="eventList">
           ...
           class="event-link flex items-center justify-between px-6 py-4 border-b border-gray-100 hover:bg-gray-50 transition no-underline group">
            <div class="flex items-center gap-4">
                <div class="w-20 h-16 rounded-lg overflow-hidden bg-gray-200 shrink-0 border border-gray-100">
                    <img src="{{ asset('images/' . $event->poster) }}" onerror="this.src='https://placehold.co/100x80?text=Event'" class="w-full h-full object-cover">
                </div>
                <div>
                    <h3 class="text-lg font-bold text-gray-800 group-hover:text-[#7a4988] transition">{{ $event->judul }}</h3>
                    <p class="text-gray-500 text-xs mt-1 font-medium flex items-center gap-2">
                        {{ $event->tanggal }} 
                        <span class="bg-gray-100 text-gray-500 px-2 py-0.5 rounded text-[10px] font-bold uppercase border border-gray-200">{{ $event->kategori }}</span>
                    </p>
                </div>
            </div>
            <div class="shrink-0">
                <span class="inline-flex items-center justify-center px-6 py-2 rounded-full border text-sm font-bold shadow-sm {{ $event->is_full ? 'bg-red-100 text-red-700 border-red-200' : 'bg-blue-100 text-blue-700 border-blue-200' }}">
                    {{ $event->total_pendaftar }}/{{ $event->kuota }} {{ $event->label }}
                </span>
            </div>
        </a>
        @empty
            <div class="text-center py-20 text-gray-400 font-bold">Belum ada event yang tersedia.</div>
        @endforelse
    </div>

    {{-- PAGINATION --}}
    <div class="bg-white px-6 py-5 flex justify-center border-t">
